...
"""
function generate_initial_model(; hSets, hData, hParameters, model=JuMP.Model(Ipopt.Optimizer))
    (; sets, parameters, data, fixed, lower, upper) = generate_starting_values(; hSets, hData, hParameters)
    mc = model_container_struct(model, data, parameters, sets, fixed, lower, upper)
    build_model!(mc)
    initialize_model!(mc)
    return mc
end