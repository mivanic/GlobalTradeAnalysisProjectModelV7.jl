    ...
    return NamedArray(
        (
            mapslices(sum,
                ...
                mapslices(sum, ifelse.(isnan.(data1["qxs"] .* data0["pcif"]), 0.0, data1["qxs"] .* data0["pcif"]), dims=3)[:, 1, :], dims=1)[1, :] .+ mapslices(sum, data1["pds"][sets["marg"], :] .* data0["qst"][sets["marg"], :], dims = 1)[1,:]
        ), sets["reg"])
end
